...
CREATE TABLE messages (id SERIAL PRIMARY KEY, thread_id INTEGER REFERENCES message_threads ON DELETE CASCADE, message_read BOOLEAN, sender_id INTEGER REFERENCES users, recipient_id INTEGER REFERENCES users, time_sent TIMESTAMP, message VARCHAR(5000));
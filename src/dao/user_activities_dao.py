class UserActivitiesDAO:
    ...
    def get_user_activity(self, username):
        sql = "SELECT user_activities.id, user_id, activity_id, start_time, end_time, activities.activity FROM user_activities INNER JOIN users ON user_id=users.id INNER JOIN activities ON activity_id=activities.id WHERE users.username=:username AND end_time IS NULL"
        result = self.db.session.execute(sql, {"username": username})
        return result.fetchone()

    def get_user_activities(self, username):
        sql = "SELECT user_activities.id, user_id, activity_id, start_time, end_time, activities.activity FROM user_activities INNER JOIN users ON user_id=users.id INNER JOIN activities ON activity_id=activities.id WHERE users.username=:username ORDER BY start_time DESC"
        result = self.db.session.execute(sql, {"username": username})
